...
// ── Modal ────────────────────────────────────────────────────────────
export function Modal({ open, title, editing, onClose, onSave, children, wide }) {
  useEffect(() => {
      ...
      <div style={{
        ...
        <div style={{ padding:'18px 24px', borderBottom:'1px solid #e2e6ed', display:'flex', alignItems:'center', justifyContent:'space-between' }}>
          <div>
            ...
            }}>{editing ? 'ALTERAR' : 'INSERIR'}</span>
          </div>
          <button onClick={onClose} style={{ background:'none', border:'none', cursor:'pointer', fontSize:20, color:'#94a3b8', padding:'2px 6px', borderRadius:4 }}>✕</button>
        </div>
