...
namespace oProjeto.Server.Controllers
{
    ...
    public class PaisesController(PaisRepository repo) : ControllerBase
    {
        [HttpGet]
        ...
        public async Task<IActionResult> Create(Paises body)
        {
            ...
            return CreatedAtAction(nameof(Get), new { id = created.CodPais }, created);
        }

        [HttpPut("{id}")]
        public async Task<IActionResult> Update(int id, Paises body)
        {
            if (id != body.CodPais) return BadRequest();
